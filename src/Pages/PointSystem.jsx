...
export default function PointSystem() { 
  ...
  return (
    <>
      ...
      />
    <div className="min-h-screen pb-14 bg-gradient-to-br from-[#1e1e1e] to-[#4e3535] text-white font-sans">
      <main className="container px-4 py-12 pt-20 mx-15 auto">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <Card className="bg-[#2a2a2a] border-[#444] shadow-lg hover:shadow-xl transition-shadow duration-300">
            <CardHeader>
              <CardTitle className="text-3xl font-bold text-[#ff3e3e]">Badge Levels</CardTitle>
              <p className="text-sm text-gray-400">Earn badges as you accumulate points</p>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-[#444]">
                    <TableHead className="text-left text-white">Level</TableHead>
                    <TableHead className="text-right text-white">Points</TableHead>
                    <TableHead className="text-right text-white">Next Level</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {badgeLevels.map((badge, index) => (
                    <TableRow key={index} className="border-b border-[#444] last:border-b-0 hover:bg-[#3a3a3a] transition-colors duration-200">
                      <TableCell className="py-3 text-white">
                        <span className={`inline-block px-2 py-1 rounded-full text-xs font-semibold ${badge.color} text-white mr-2`}>
                          {badge.level}
                        </span>
                      </TableCell>
                      <TableCell className="text-right text-[#ff3e3e] font-bold py-3">{badge.points}</TableCell>
                      <TableCell className="py-3 text-right text-gray-400">{badge.nextLevel ? `${badge.nextLevel} points` : 'Max level'}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
