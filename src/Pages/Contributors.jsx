...
const ContributorsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchContributors = async () => {
      try {
        const response = await fetch(`/api/projects/contributors/${repoName}`);
        if (!response.ok) {
          throw new Error("Server was unable to reach...");
        }
        const data = await response.json();

        if (data.length > 0) {
          const contributorsWithPics = await Promise.all(
            data.map(async (contributor) => {
              const githubResponse = await fetch(`https://api.github.com/users/${contributor.name}`);
          ...
          setContributors(contributorsWithPics);
        } else {
          setContributors([]); 
        }
      } catch (e) {
