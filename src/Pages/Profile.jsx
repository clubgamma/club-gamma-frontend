        ...
        <Tooltip delayDuration={200}>
          <TooltipTrigger asChild>
            <div
              ...
              aria-label={`${value || "No"} contribution${
                value !== 1 ? "s" : ""
              } on ${date}`}
            />
          </TooltipTrigger>
          <TooltipContent>
            <p className="font-dm-sans">
              {value || "No"} contribution{value !== 1 ? "s" : ""} on {date}
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
...
export default function GitHubProfile() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const { user, stats } = await Global.httpGet(
              ...
              <div className="space-y-3">
                {userData.prs.map((pr, index) => (
                  ...
                  >
                    <Card
                      key={index}
                      className="bg-[#1e1e1e]/50 border-[#4e3535] hover:border-red-900 transition-all duration-300"
                    >
                      <CardContent className="p-4">
