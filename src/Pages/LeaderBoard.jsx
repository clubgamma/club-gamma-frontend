...
const LeaderBoard = () => {
    ...
    return (
        ...
        <main className="container px-4 py-8 mx-auto">
          {currentUser && (
            <div className="bg-[#2A2A2A] rounded-xl shadow-lg p-4 md:p-6 border border-[#3A3A3A] mb-8">
              <div className="flex flex-col items-center justify-between space-y-4 md:flex-row md:space-y-0">
                <div className="flex flex-col items-center space-y-4 md:flex-row md:space-y-0 md:space-x-4">
                  ...
                  <div className="text-center md:text-left">
                    <h2 className="text-2xl font-bold text-white md:text-3xl">{Global.user.name}</h2>
                    <h2
                ...
                <div className="grid w-full grid-cols-3 gap-2 text-center md:gap-4 md:w-auto">
                  <div className="p-2 md:p-4 rounded-lg bg-[#3A3A3A]">
                    <div className="text-xl md:text-3xl font-bold text-[#FF4545]">{currentUser.points}</div>
                    <div className="text-xs text-gray-300 md:text-sm">Points</div>
                  </div>
                  <div className="p-2 md:p-4 rounded-lg bg-[#3A3A3A]">
                    <div className="text-xl md:text-3xl font-bold text-[#FFA500]">{currentUser.totalPRs}</div>
                    <div className="text-xs text-gray-300 md:text-sm">Total PRs</div>
                  </div>
                  <div className="p-2 md:p-4 rounded-lg bg-[#3A3A3A]">
                    <div className="text-xs font-medium text-gray-300 md:text-sm">PRs Status</div>
          ...
          <div className="bg-[#2A2A2A] rounded-xl shadow-lg border border-[#3A3A3A]">
            <div className="p-4 md:p-6 border-b border-[#3A3A3A]">
              <div className="flex flex-col items-center justify-between space-y-4 md:flex-row md:space-y-0">
                <div className="flex items-center space-x-4">
                  <h2 className="text-2xl font-bold text-white md:text-3xl">Leaderboard</h2>
                  <Button
                    variant="outline"
                    size="sm"
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#3A3A3A]">
                  <tr>
                    <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-gray-300 uppercase">Rank</th>
                    <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-gray-300 uppercase">User</th>
                    <th className="px-4 py-3 text-xs font-medium tracking-wider text-right text-gray-300 uppercase">Points</th>
                    <th className="px-4 py-3 text-xs font-medium tracking-wider text-right text-gray-300 uppercase">PRs</th>
                  </tr>
                </thead>
                        ...
                        <div className="flex items-center">
                          {contributor.rank <= 3 ? (
                            <TrophyIcon className={`h-6 w-6 ${
                              contributor.rank === 1 ? 'text-[#FFD700]' :
            ...
            <div className="p-4 border-t border-[#3A3A3A]">
              <div className="flex flex-col items-center justify-between space-y-4 md:flex-row md:space-y-0">
                <Select value={limit.toString()} onValueChange={handleLimitChange}>
                  <SelectTrigger className="w-full md:w-[180px] bg-[#3A3A3A] border-[#4A4A4A] text-white">
